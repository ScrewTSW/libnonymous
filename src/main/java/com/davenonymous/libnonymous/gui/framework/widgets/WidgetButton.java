...
public class WidgetButton extends Widget {
    ...
    public void draw(Screen screen, PoseStack matrixStack) {
        ...
        if(atlasSprite != null) {
            ...
            screen.getMinecraft().getTextureManager().bindForSetup(backgroundTexture);
            GUIHelper.drawModalRectWithCustomSizedTexture(0, 0, 0, 0, width, height, 16.0f, 16.0f);
        }

        RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, hovered ? 1.0F : 1.0F);
        screen.getMinecraft().getTextureManager().bindForSetup(GUI.tabIcons);
    ...
    protected void drawButtonContent(PoseStack matrixStack, Screen screen, Font renderer) {
        drawString(matrixStack, screen, renderer);
    }

    protected void drawString(PoseStack matrixStack, Screen screen, Font renderer) {
        int color = 0xFFFFFF;
        GuiComponent.drawCenteredString(matrixStack, renderer, unlocalizedLabel, width / 2, (height - 8) / 2, color);
    }
